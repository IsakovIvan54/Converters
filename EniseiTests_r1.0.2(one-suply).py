...
SUPPLY = rm.open_resource('USB0::0x1AB1::0x0E11::DP8A244400389::INSTR') # 
...
# Настройка источника
SUPPLY.write(':SOUR2:CURR 2')
SUPPLY.write(':SOUR3:CURR 2')
...
def run_TEST(INVOLT, OUTCURR):
    ...
    time.sleep(1)
    SUPPLY.write(':SOUR2:VOLT ' + str(INVOLT/2))
    SUPPLY.write(':SOUR3:VOLT ' + str(INVOLT/2))
    # SUPPLY.write('SOUR:VOLT ' + str(INVOLT))
    LOAD.write('CURR ' + str(OUTCURR))
    # CONTROL_SUPPLY.write(f':SOUR3:VOLT {DVolt}')
    ...
    currentHH = float(SUPPLY.query('MEAS:CURR? CH2')) # Входный ток без нагрузки
    voltageHH = float(SUPPLY.query('MEAS:VOLT? CH2')) - 1 * float(SUPPLY.query('MEAS:VOLT? CH3')) # Входное напряжение без нагрузки
    time.sleep(2)
    voltageHHout = float(MULTIMETR.query(':MEAS:VOLT:DC?')) # Выходное напряжение без нагрузки
    time.sleep(0.2)
    LOAD.write(':INP ON')
    time.sleep(3)
    currentLOAD = float(SUPPLY.query('MEAS:CURR? CH2')) # Входный ток c нагрузкой
    voltageLOAD = float(SUPPLY.query('MEAS:VOLT? CH2')) - 1 * float(SUPPLY.query('MEAS:VOLT? CH3')) # Входное напряжение c нагрузкой
